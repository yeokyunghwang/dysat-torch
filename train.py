...
def normalize_gcn(adj):
    """D^-1/2 (A+I) D^-1/2, as train.py does before the model sees the graph."""
    adj = sp.coo_matrix(adj, dtype=np.float32) + sp.eye(adj.shape[0], dtype=np.float32)
    d = np.asarray(adj.sum(1)).flatten()
    dinv = sp.diags(np.power(d, -0.5, where=d > 0, out=np.zeros_like(d)))
    return adj.dot(dinv).transpose().dot(dinv).tocoo()
